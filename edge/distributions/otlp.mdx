...
Edge OTLP targets OpenTelemetry environments. It supports the
...
Edge works with the standard OTLP data model. OTLP fields map to policy
matchers as follows.
...
```json
{
  ...
  "log": {
    "match": [{ "scope_attribute": "library.name", "exact": "noisy-http-client" }],
    "keep": "none"
  }
}
```
