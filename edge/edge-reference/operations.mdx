...
| Level   | Description                        |
| ------- | ---------------------------------- |
| `debug` | Most verbose, debugging information |
...
```bash
curl http://localhost:8080/_edge/metrics
```

Point a Prometheus scrape at this endpoint to monitor Edge itself (request
counts, request durations, policy batch stats, build info). This is separate
from any `/metrics` traffic Edge filters and forwards to your upstream.
