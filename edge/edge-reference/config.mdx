...
- **Optional fields accept an empty string to mean "unset".** `TERO_METRICS_URL=`
  sets `metrics_url` back to null rather than to an empty URL.
- **An unparseable value fails startup.** `TERO_LOG_LEVEL=bogus` or a
  non-numeric `TERO_LISTEN_PORT` exits with `config.load.error err="InvalidValue"`
  instead of silently falling back to the default.
- **Lists are not overridable.** `policy_providers`,
  `service.resource_attributes`, and `service.labels` are arrays, so they have no
  env var — supply a config file, or use
