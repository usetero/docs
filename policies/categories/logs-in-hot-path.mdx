...
This reference describes the logs in hot path policy category.

## Definition

A hot-path log is emitted unconditionally from code that runs frequently during normal operation. Common sources include request middleware, item-processing loops, polling loops, and other code paths that execute for most requests or records.
...
Tero identifies hot-path logs primarily by relative volume. If one event shape represents a disproportionate share of a service's logs, the event is treated as a hot-path candidate.
