...
One log event with a 10KB response body, fired 100,000 times a day, is 1GB of storage for a single log pattern.
...
Unlike infrastructure noise, excessive payloads usually come from intentional logging decisions. The right fix is often to log less in the first place. The developer knows whether that response body helps debugging or operations.

## How it works

Tero flags fields that are too large for routine logs: full HTTP response bodies, entire serialized objects, and 500-line stack traces. These fields add kilobytes to each matching log event.
