...
Nobody intends to leak PII. But it happens. And once it's in your observability platform, you have a compliance problem.

## How Tero detects it

Tero scans field values for PII patterns. Every field that could possibly contain sensitive data gets scanned—nothing slips through.

The only fields Tero skips are ones where scanning is pointless: timestamps, log levels, status codes, PIDs, port numbers. There's no reason to scan a timestamp for credit card numbers, or check a severity level for email addresses.

Everything else gets scanned. Message bodies, user-facing fields, request payloads, string fields of any kind. If PII could be there, Tero looks for it.
...
Edge enforcement is critical. Redacting at the edge means sensitive data never leaves your infrastructure. No cleanup required. No breach notifications. The blast radius is zero because the data never reached your provider.
