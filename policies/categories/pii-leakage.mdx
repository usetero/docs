...
Tero uses the [Master Catalog](/master-catalog) to understand each field: what it represents, what values it contains, why it exists. This determines which fields get scanned for PII.

Any field that could contain PII through normal developer error gets scanned: `error_message`, `request_body`, `user_input`, free-form text fields. Fields that are structurally incompatible with PII are skipped: `timestamp`, `severity`, `status_code`, `pid`. These have fixed formats. PII can't end up there through a logging mistake.
