...
The only fields Tero skips are ones where scanning is pointless: timestamps, log levels, status codes, PIDs, port numbers. There's no reason to scan a timestamp for credit card numbers, or check a severity level for email addresses. Everything else gets scanned.
