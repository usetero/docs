...
Rules are organized by [category](/data-quality): redundant attributes, health check logs, stray debugging, verbose payloads, repetitive events, PII leakage. Each category has a risk level. Start with the obvious: redundant fields that duplicate other fields, debug logs that shouldn't be in production. These are zero-risk. You'll agree immediately.
