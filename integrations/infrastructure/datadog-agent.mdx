...
<Steps>
  ...
  <Step title="Create the Edge ConfigMap">
    Create a ConfigMap with your Edge configuration and policies:
    ...
    <Note>
    Adjust `upstream_url` for your [Datadog region](https://docs.datadoghq.com/getting_started/site/). For example, use `https://agent-http-intake.logs.us5.datadoghq.com` for US5.
    </Note>
  ...
  <Step title="Verify">
    ...
    ```bash
    kubectl get pods -l app=tero-edge
    ```

    Check Edge logs for incoming traffic:

    ```bash
    kubectl logs -l app=tero-edge --tail=50
    ```
...
```bash
kubectl get pods -l app=tero-edge -o wide
```
...
```bash
kubectl logs -l app=tero-edge | grep -i policy
```
