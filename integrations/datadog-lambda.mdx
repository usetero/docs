...
<Note>
  FIPS compliance is not currently supported. Reach out to [Tero
...
- Tero account
- Datadog Application key (create one in
  [Organization Settings → Application Keys](https://app.datadoghq.com/organization-settings/application-keys))

<Note>
  Create a new Application key specifically for Tero. Don't reuse existing
  keys—this makes it easier to rotate or revoke access later.
</Note>

## Quick deploy

Deploy the Tero Lambda Extension stack with one click.

<Card
  title="Launch Stack"
  icon="aws"
  href="https://console.aws.amazon.com/cloudformation/home#/stacks/create/review?templateURL=https://raw.githubusercontent.com/usetero/datadog-lambda-extension/main/serverless.yaml"
  horizontal
>
  Deploy via AWS CloudFormation
</Card>

You'll need your Datadog API key, Datadog Application key, and Tero API key
during setup.

## Connect

For manual setup or to integrate into existing infrastructure, follow these
steps.
...
<Steps>
  ...
  <Step title="Add the Lambda layer">
    ...
    **AMD64:**
    ...
    <Note>
    Replace `<version>` with the latest version number. Check the [releases](https://github.com/usetero/datadog-lambda-extension/releases) for current versions.
    </Note>
...
<Tabs>
  ...
  <Tab title="AWS CLI">
    ...
    aws lambda update-function-configuration \
      ...
      --layers "arn:aws:lambda:us-east-1:242046726909:layer:Tero-Datadog-Extension-ARM:1" \
