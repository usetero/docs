...
before Prometheus scrapes them. Edge sits between Prometheus and your
application, applying policies as each scrape passes through.

## How it works

Edge runs as a sidecar container in the same pod as your application. Prometheus
scrapes Edge instead of your application. Edge proxies the request to your app's
`/metrics` endpoint, applies policies to filter metrics, and returns the
filtered response.
...
Fetch policies from a remote endpoint. Use this for dynamic policies managed via
the Tero API.
