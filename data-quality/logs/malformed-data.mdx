...
Tero evaluates each [log event](/context/log-events) in your context graph. It looks at the content structure and whether the log is parseable.

When Tero finds malformed data (binary content, unparseable strings, corrupted output), it creates a rule for that specific log event. You approve the rule, and Tero generates a policy that matches the exact pattern it found.
