...
Tero scans log content for patterns that match sensitive data: credit card numbers (Luhn algorithm), email addresses, SSNs, API keys, JWTs, and other common PII patterns.

When Tero finds PII, it identifies the field and the service. You can then redact (replace with `[REDACTED]`), hash (replace with a consistent hash for correlation), or drop the field entirely.
