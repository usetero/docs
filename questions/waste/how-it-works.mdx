...
Chances are you've done manual waste analysis. Maybe before a renewal, or your boss wanted a service's log costs down. Why were you chosen? Probably because you have the deepest understanding of the service.

Maybe you wrote the code, or added the instrumentation. Maybe you just operate it. Either way, you know the context: core functions, where it sits in your architecture, which services depend on it, how it breaks. When you look at a log, you can assess whether it's waste. Not binary, a gradient. Obvious garbage is easy. Nuanced calls require understanding the service, the event, the situation where someone might actually need it.

But the process is slow, manual, and has opportunity cost. You do it quarterly at best, limited to the obvious stuff. The rest keeps costing you, month after month.
...
<Steps>
  ...
  <Step title="Attribute analysis">
    For logs we keep, we analyze attributes. Duplicate values, static metadata, low-value fields. Trim the noise, keep the event.
  </Step>
</Steps>

This is excessive by design. Your manual analysis can't do this at scale. Ours runs continuously, across every log event, all the time.
...
After analyzing billions of logs, patterns emerge. Certain types of waste keep showing up. We call these categories, but they're not limits. We don't start with categories, we observe them after analysis completes.

The reasoning is unconstrained. Categories are how we organize the output so you can understand it, control it, and build trust incrementally.

<AccordionGroup>
  <Accordion title="Junk" icon="file-circle-xmark">
    Malformed data, binary blobs, accidental output. `fdsfdsfdseef`. Logs that convey nothing useful. Safe to drop entirely.
  </Accordion>
  <Accordion title="Debug output" icon="bug">
    Developer markers that shouldn't be in production. `GOT HERE!`, `TODO: remove this`, print statements left behind. Safe to drop entirely.
  </Accordion>
  <Accordion title="External probes" icon="heart-pulse">
    Health checks, readiness probes, synthetic monitoring hitting your service from outside. Infrastructure talking to itself. Safe to drop. (Health check *failures* are different, those have investigative value.)
  </Accordion>
  <Accordion title="Noisy signals" icon="volume-high">
    High volume, low variance. Cache hits, heartbeats, routine operations. 1000 cache hits means the cache works. One situation, not 1000 investigations. Sample to reduce volume without losing visibility.
  </Accordion>
  <Accordion title="Correlated errors" icon="server">
    A thousand timeout errors during an outage are symptoms of one problem, not a thousand separate incidents. Sample to capture the pattern without the flood.
  </Accordion>
  <Accordion title="Redundant attributes" icon="copy">
    The log matters, some attributes don't. Stack traces when the error code tells the story. Full request bodies when the endpoint is enough. Trim the attribute, keep the event.
  </Accordion>
  <Accordion title="Static metadata" icon="tag">
    SDK versions, infrastructure plumbing, values that never change and never help debugging. Trim the attribute, keep the event.
  </Accordion>
  ...
  <Accordion title="Uncategorized" icon="question">
...
Not everything is waste. A payment failure with a different user each time? 1000 customers with 1000 problems. Keep all of them. Business events, customer-specific errors, state changes. Each instance is a distinct situation worth investigating. Tero identifies these and leaves them alone.
...
When you're ready to act on what Tero found, see [Taking action](/questions/waste/taking-action).
