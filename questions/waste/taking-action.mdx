...
|----------|----------------------|
| Junk, debug output | Code — shouldn't exist |
| External probes | Infrastructure — filter |
| Noisy signals | Infrastructure — sample |
| Correlated errors | Infrastructure — sample |
| Redundant attributes | Infrastructure — trim |

Junk and debug output are code problems. The fix is to stop emitting them. Probes, noisy signals, and correlated errors are fine to emit — you just don't need to store all of them. Filter or sample in your infrastructure.

You can configure Tero to route automatically. Junk and debug open PRs. Probes and noisy signals push policies. You review what needs judgment, the obvious stuff flows without intervention.
